...
export async function vendorLogin(email: string, password: string): Promise<{ success: boolean; token?: string; user?: User; message?: string }> {
  try {
    ...
    return { success: false, message: 'Network error during vendor sign in.' };
  }
}

export async function deliveryLogin(identifier: string, password: string): Promise<{ success: boolean; token?: string; user?: User; message?: string }> {
  try {
    const res = await fetch('/api/auth/delivery/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ identifier, password }) });
    return await res.json();
  } catch {
    return { success: false, message: 'Network error during rider sign in.' };
  }
}
...
export async function getStallById(id: string, token?: string): Promise<Stall | null> {
  try {
    const res = await fetch(`/api/stalls/${id}`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
    if (!res.ok) throw new Error('API error');
    const json = await res.json();
    return json.data;
  } catch {
...
export async function getVendorStall(token?: string): Promise<{ success: boolean; data?: Stall; message?: string }> {
  try {
    const res = await fetch('/api/stalls/vendor/my-stall', {
      headers: token ? { Authorization: `Bearer ${token}` } : {}
    });
    return await res.json();
  } catch {
    return { success: false, message: 'Network error loading your assigned stall.' };
  }
}
...
export async function updateDeliveryStatusApi(
  deliveryId: string,
  status: 'accepted' | 'arrived_at_vendor' | 'out_for_delivery' | 'arrived_at_customer',
  token?: string
): Promise<{ success: boolean; message?: string; data?: Delivery }> {
  try {
    ...
    return { success: false, message: 'Network error updating delivery status.' };
  }
}

export async function updateRiderPresenceApi(online: boolean, token?: string): Promise<{ success: boolean; online?: boolean; message?: string }> {
  try {
    const res = await fetch('/api/deliveries/presence', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) },
    ...
    return { success: false, message: 'Network error updating rider availability.' };
  }
}

export async function verifyDeliveryOtpApi(deliveryId: string, otp: string, token?: string): Promise<{ success: boolean; message?: string; data?: Delivery }> {
  try {
    const res = await fetch(`/api/deliveries/${deliveryId}/delivery-otp/verify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) },
      body: JSON.stringify({ otp })
    });
    ...
    return { success: false, message: 'Network error verifying Delivery OTP.' };
  }
}

export async function getDeliveryOtpApi(orderId: string, token?: string): Promise<{ success: boolean; data?: { otpCode: string; expiresAt: number }; message?: string }> {
  try {
    const res = await fetch(`/api/orders/${orderId}/delivery-otp`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
    return await res.json();
  } catch {
    return { success: false, message: 'Network error loading Delivery OTP.' };
  }
}

export async function getAdminAssignments(token?: string): Promise<{ success: boolean; availableRiders: { id: string; name: string; rating: number; vehicle: string }[]; data: any[]; message?: string }> {
  try {
    const res = await fetch('/api/deliveries/admin/assignments', { headers: token ? { Authorization: `Bearer ${token}` } : {} });
    return await res.json();
  } catch {
    return { success: false, availableRiders: [], data: [], message: 'Network error loading delivery assignments.' };
  }
}

export async function assignRiderApi(orderId: string, riderId: string, token?: string): Promise<{ success: boolean; message?: string; data?: Delivery }> {
  try {
    const res = await fetch('/api/deliveries/admin/assign', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) },
    ...
    return { success: false, message: 'Network error assigning rider.' };
  }
}

export async function declineDeliveryApi(deliveryId: string, token?: string): Promise<{ success: boolean; message?: string; data?: Delivery }> {
  try {
    const res = await fetch(`/api/deliveries/${deliveryId}/decline`, {
      method: 'POST',
      headers: token ? { Authorization: `Bearer ${token}` } : {}
    });
    return await res.json();
  } catch {
    return { success: false, message: 'Network error declining delivery.' };
  }
}

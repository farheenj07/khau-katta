...
export const RoleProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [currentRole, setCurrentRole] = useState<UserRole>(() => {
    ...
    return saved && ['customer', 'admin', 'delivery_partner', 'stall_owner', 'vendor'].includes(saved) ? saved : 'customer';
  });
  ...
  const roleMeta: Record<UserRole, { title: string; defaultUser: string }> = {
    ...
    stall_owner: { title: 'Stall Vendor', defaultUser: 'Khau Katta Stall Owner' },
    vendor: { title: 'Stall Vendor', defaultUser: 'Khau Katta Stall Owner' }
  };
  ...
  const value: RoleContextType = {
    ...
    isVendor: currentRole === 'vendor' || currentRole === 'stall_owner',

...
import { assignRiderApi, getAdminAssignments, getAdminStats, getAdminReviews, moderateReview, getStalls } from '../../services/api';
...
export const AdminDashboard: React.FC = () => {
  ...
  const [availableRiders, setAvailableRiders] = useState<{ id: string; name: string; rating: number; vehicle: string }[]>([]);
  ...
  useEffect(() => {
    ...
    if (activeTab === 'delivery') {
      getAdminAssignments(token || undefined).then(result => {
        if (result.success) {
          setAssignmentData(result.data);
          setAvailableRiders(result.availableRiders);
        }
      });
    }
  ...
  const assignRider = async (orderId: string) => {
    ...
    if (!riderId) return;
    const result = await assignRiderApi(orderId, riderId, token || undefined);
    if (!result.success) {
      showToast(result.message || 'Unable to assign rider.', 'error');
      return;
    }
    showToast(result.message || 'Rider assigned.', 'success');
    const refreshed = await getAdminAssignments(token || undefined);
    if (refreshed.success) {
      setAssignmentData(refreshed.data);
      setAvailableRiders(refreshed.availableRiders);
    }
  };
      ...
      <div className="flex-1 flex flex-col md:flex-row">
        ...
        <main className="flex-1 p-6 md:p-8 space-y-8 overflow-y-auto">
          ...
          {activeTab === 'delivery' && (
            <div className="space-y-6">
              <div>
                <h2 className="text-2xl font-black text-[#2e1b10] tracking-tight">Live Delivery Assignments</h2>
                <p className="text-xs text-[#735442]">Assign available online riders to orders that require delivery.</p>
              </div>

              <div className="grid gap-4 lg:grid-cols-2">
                {assignmentData.map(item => (
                  <div key={item.id} className="bg-[#fffdfb]/95 rounded-2xl p-5 border border-[#eed7c2] shadow-xs space-y-3">
                    <div className="flex items-center justify-between gap-3">
                      <span className="font-mono text-xs font-black text-[#c86228]">{item.orderNumber}</span>
                      <span className="text-[10px] font-black uppercase rounded-full bg-amber-100 px-2 py-1 text-amber-800">{item.delivery?.status || 'unassigned'}</span>
                    </div>
                    <p className="text-sm font-black text-[#2e1b10]">{item.customerName} · {item.stallName}</p>
                    <p className="text-xs text-[#735442]">₹{item.totalAmount} · {item.deliveryAddress}</p>
                    <div className="flex gap-2">
                      <select value={selectedRiders[item.id] || ''} onChange={event => setSelectedRiders(previous => ({ ...previous, [item.id]: event.target.value }))} className="min-w-0 flex-1 rounded-xl border border-[#eed7c2] bg-[#fdf8f3] px-3 py-2 text-xs font-bold">
                        <option value="">Select available rider</option>
                        {availableRiders.map(rider => <option key={rider.id} value={rider.id}>{rider.name} · {rider.vehicle} · {rider.rating}</option>)}
                      </select>
                      <button type="button" onClick={() => assignRider(item.id)} className="rounded-xl bg-[#c86228] px-4 py-2 text-xs font-black text-white">Assign Rider</button>
                    </div>
                  </div>
                ))}
              </div>
              {!assignmentData.length && <div className="rounded-2xl border border-[#eed7c2] bg-[#fffdfb] p-6 text-center text-xs text-[#735442]">No uncompleted delivery orders currently require assignment.</div>}
            </div>
          )}

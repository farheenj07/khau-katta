...
import { getDeliveryTracking, stepSimulationApi, toggleSimulationApi } from '../services/api';
...
export const LiveTrackingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isSimulating) {
      simInterval = setInterval(async () => {
        const res = await stepSimulationApi(deliveryId);
  ...
  const optDist = delivery.route?.optimizedDistanceKm ?? 1.8;
  const optDur = delivery.route?.optimizedDurationMins ?? 8;
  const progress = delivery.simulationProgress ?? 0;
  const remainingDist = Number((optDist * (1 - progress)).toFixed(1));
  const remainingTime = Math.max(1, Math.round(optDur * (1 - progress)));
      ...
      <div className="bg-gradient-to-r from-[#291305] via-[#3d2314] to-[#291305] text-white rounded-3xl p-6 sm:p-8 shadow-xl border border-[#f0bd9b]/20 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
        <div className="space-y-2">
          ...
          <h2 className="font-serif text-2xl sm:text-4xl font-black text-white">
            {isDelivered ? 'Your Order Has Arrived! 🎉' : `Arriving in ~${remainingTime} Minutes`}
          </h2>
          <p className="text-xs text-[#eed7c2]/90 flex items-center gap-2">
            <Navigation size={14} className="text-amber-400" />
            <span>{isDelivered ? 'Delivered to your doorstep' : `${remainingDist} km remaining from Club Road Khau Katta Hub`}</span>
            <span>•</span>
            <span className="text-emerald-400 font-mono">Updated just now</span>
          </p>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-[#7c4d2e] font-mono font-bold">
            Coordinates: {delivery?.currentLat ?? 15.8580}, {delivery?.currentLng ?? 74.5080}
          </span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 bg-[#fff8f2]/95 rounded-3xl p-6 sm:p-8 border border-[#f0bd9b] shadow-xs space-y-6">
          ...
          <div className="space-y-6 relative before:absolute before:left-4 before:top-3 before:bottom-3 before:w-0.5 before:bg-[#f0bd9b]">
            {[
              { status: 'confirmed', title: 'Order Confirmed', desc: 'Vendor accepted customer order & items verified', done: true },
              { status: 'preparing', title: 'Order Being Prepared', desc: 'Fresh Belagavi delicacy cooked in kitchen', done: true },
              { status: 'ready_for_pickup', title: 'Order Picked Up (OTP Verified)', desc: 'Rider verified 6-digit pickup OTP at stall', done: delivery?.status !== 'assigned' && delivery?.status !== 'accepted' },
              { status: 'out_for_delivery', title: 'Out for Delivery', desc: 'Rider Ramesh Naik navigating via Camp Road', done: delivery?.status === 'out_for_delivery' || isDelivered, current: delivery?.status === 'out_for_delivery' },
              { status: 'delivered', title: 'Delivered to Customer', desc: 'Handed over at Sai Residency doorstep', done: isDelivered, current: isDelivered }
            ].map((step, idx) => (
          ...
          {/* INTELLIGENT ROUTE OPTIMIZATION COMPARISON CARD */}
          <div className="p-5 rounded-2xl bg-[#fff0e2] border border-[#f0bd9b] space-y-3 text-xs text-[#3c1e0a]">
            <div className="flex items-center justify-between font-bold border-b border-[#f0bd9b]/60 pb-2">
              <span className="flex items-center gap-1.5 text-[#b85018]">
                <TrendingUp size={15} />
                <span>AI Intelligent Route Optimization Analysis (VRP / A*)</span>
              </span>
              <span className="text-[10px] bg-emerald-100 text-emerald-800 font-bold px-2 py-0.5 rounded">
                Saved {delivery.route?.timeSavedMins ?? 4} Mins
              </span>
            </div>

            <div className="grid grid-cols-3 gap-2 text-center pt-1">
              <div className="p-2 bg-[#fff8f2] rounded-xl border border-[#f0bd9b]/50">
                <span className="text-[10px] text-[#7c4d2e] block">Distance Saved</span>
                <span className="font-mono font-bold text-[#b85018] text-sm">{delivery.route?.distanceSavedKm ?? 0.6} km</span>
              </div>
              <div className="p-2 bg-[#fff8f2] rounded-xl border border-[#f0bd9b]/50">
                <span className="text-[10px] text-[#7c4d2e] block">Time Saved</span>
                <span className="font-mono font-bold text-[#b85018] text-sm">{delivery.route?.timeSavedMins ?? 4} mins</span>
              </div>
              <div className="p-2 bg-[#fff8f2] rounded-xl border border-[#f0bd9b]/50">
                <span className="text-[10px] text-[#7c4d2e] block">Fuel Saved</span>
                <span className="font-mono font-bold text-emerald-700 text-sm">{delivery.route?.fuelSavedLiters ?? 0.024} L</span>
              </div>
            </div>
          </div>
        </div>

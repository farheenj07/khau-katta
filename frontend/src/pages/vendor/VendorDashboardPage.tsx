...
import { Stall, Product, StallCategory, Order } from '../../types';
import {
  getStalls,
  getStallById,
  getCategories,
...
export const VendorDashboardPage: React.FC = () => {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      if (stallsData.length > 0) {
        const initialStallId = stallsData[0].id;
  ...
  const handleToggleShopOpen = async () => {
    ...
    try {
      const res = await toggleStallOpenStatusApi(currentStall.id, newOpenState);
      if (res.success && res.data) {
        setCurrentStall(res.data);
        // Also update in stalls list
        setStallsList(prev =>
          prev.map(s => (s.id === currentStall.id ? { ...s, isOpen: newOpenState } : s))
        );
        showToast(
  ...
  const handleToggleAvailability = async (prod: Product) => {
    ...
    try {
      const res = await toggleProductAvailabilityApi(prod.id, newAvail);
      if (res.success) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#fff8f2]/95 rounded-3xl p-6 border border-[#f0bd9b] shadow-xs backdrop-blur-sm">
        <div>
          ...
          <p className="text-xs text-[#7c4d2e] mt-1">
            Toggle shop open/closed status, add menu dishes, update prices, and control real-time inventory
          </p>
        </div>

        {/* Stall Selector Dropdown */}
        <div className="flex items-center gap-2">
          <label className="text-xs font-bold text-[#3c1e0a] flex-shrink-0">
            Active Vendor Shop:
          </label>
          <select
            value={selectedStallId}
            onChange={e => setSelectedStallId(e.target.value)}
            className="p-2.5 bg-[#fff0e2] text-xs font-bold text-[#3c1e0a] rounded-2xl border border-[#f0bd9b] focus:outline-none focus:border-[#b85018] shadow-xs cursor-pointer min-w-[220px]"
          >
            {stallsList.map(s => (
              <option key={s.id} value={s.id}>
                Stall {s.stallNumber}: {s.name} ({s.isOpen ? 'OPEN' : 'CLOSED'})
              </option>
            ))}
          </select>
        </div>
                ...
                {vendorOrders.map(order => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* PROMINENT PICKUP OTP BADGE (When status === 'ready_for_pickup') */}
                          ...
                          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-[#1b0d04] p-4 rounded-xl border border-[#f0bd9b]/30">
                            <div>
                              <span className="text-[10px] uppercase font-bold text-[#eed7c2]/70 block">
                                Share with Delivery Partner (Ramesh Naik):
                              </span>

...
export function getOrCreatePickupOtp(orderId: string, stallId: string): PickupVerificationRecord {
  ...
  if (existing && !existing.isVerified && existing.expiresAt > now) {
    return existing;
  }
...
export function verifyPickupOtp(
  ...
  if (order.status !== 'ready_for_pickup') {
    if (order.status === 'picked_up' || order.status === 'out_for_delivery' || order.status === 'delivered') {
      return { success: true, message: 'Order has already been verified and picked up.', order };
    }
  ...
  if (record.otpCode !== trimmedInput) {
    record.attempts += 1;
...
export function generateRouteOptimization(
  ...
  // Standard route via arterial roads
  const standardPath: DeliveryWaypoint[] = [
    { lat: startLat, lng: startLng, name: 'Vendor Stall Pickup' },
    { lat: Number((startLat + (endLat - startLat) * 0.33).toFixed(4)), lng: Number((startLng + (endLng - startLng) * 0.25).toFixed(4)), name: 'Club Road Signal' },
    { lat: Number((startLat + (endLat - startLat) * 0.66).toFixed(4)), lng: Number((startLng + (endLng - startLng) * 0.75).toFixed(4)), name: 'Camp Circle' },
    { lat: endLat, lng: endLng, name: 'Customer Destination' }
  ];

  // AI Optimized path avoiding traffic bottlenecks
  const optimizedPath: DeliveryWaypoint[] = [
    { lat: startLat, lng: startLng, name: 'Vendor Stall Pickup' },
    { lat: Number((startLat + (endLat - startLat) * 0.40).toFixed(4)), lng: Number((startLng + (endLng - startLng) * 0.45).toFixed(4)), name: 'Khau Katta Express Lane' },
    { lat: Number((startLat + (endLat - startLat) * 0.80).toFixed(4)), lng: Number((startLng + (endLng - startLng) * 0.85).toFixed(4)), name: 'Camp Bypass Junction' },
    { lat: endLat, lng: endLng, name: 'Customer Destination' }
  ];
...
export const deliveries: Delivery[] = [
  {
    ...
    partnerId: 'usr-deliv-01',
    partnerName: 'Ramesh Naik',
    partnerPhone: '9740098765',
    partnerVehicleNumber: 'KA-22-EX-4589',

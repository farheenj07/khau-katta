...
export const users: User[] = [
  ...
  {
    id: 'usr-vendor-01',
    roleId: 'role-owner-004',
    name: 'Maya Traders',
    email: 'vendor@khaukatta.in',
    phone: '9876543210',
    status: 'active',
  ...
  },
  {
    id: 'usr-vendor-02',
    roleId: 'role-owner-004',
    name: 'Shahapur Handlooms',
    email: 'vendor2@khaukatta.in',
    phone: '9876543211',
    status: 'active',
  ...
  },
  {
    ...
    roleId: 'role-deliv-003',
    name: 'Sameer Patil',
    email: 'sameer.delivery@khaukatta.in',
    phone: '9740098766',
    status: 'active',
...
// Rider profiles mirror the existing delivery_partners relationship in the
// SQL schema. Delivery assignments continue to retain the authenticated user
// id, which is what authorization checks use throughout the API.
export const deliveryPartners: DeliveryPartner[] = [
  { id: 'partner-01', userId: 'usr-deliv-01', vehicleType: 'scooter', vehicleNumber: 'KA-22-EX-4589', drivingLicenseNumber: 'KA-DL-001', status: 'approved', isAvailable: true, rating: 4.85, totalDeliveries: 142 },
  { id: 'partner-02', userId: 'usr-deliv-02', vehicleType: 'motorcycle', vehicleNumber: 'KA-22-BK-2410', drivingLicenseNumber: 'KA-DL-002', status: 'approved', isAvailable: true, rating: 4.72, totalDeliveries: 98 }
];
...
export const orders: Order[] = [
  ...
  {
    id: 'ord-005',
    orderNumber: 'KK-2026-00180',
    userId: 'usr-cust-01', // Pooja Kulkarni
    stallId: 'stall-01', // Belgaum Kunda & Sweets House
    addressId: 'addr-01',
    subtotal: 520,
    discountAmount: 0,
    deliveryFee: 30,
    taxAmount: 0,
    totalAmount: 550,
    status: 'ready_for_pickup', // Ready for rider pickup!
...
export function createProduct(productData: Partial<Product>): Product {
  ...
  const stall = stalls.find(s => s.id === productData.stallId);

  const newProduct: Product = {
    id,
    stallId: productData.stallId || stalls[0]?.id || 'stall-01',
    name: productData.name?.trim() || 'New Delicious Dish',
    description: productData.description?.trim() || 'Authentic Khau Katta specialty prepared fresh.',
    price: typeof productData.price === 'number' && productData.price >= 0 ? productData.price : 100,
    ...
    imageUrl: productData.imageUrl?.trim() || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=400&auto=format&fit=crop',
    badge: productData.badge?.trim() || undefined,
    preparationTimeMins: productData.preparationTimeMins || 10,
    stallName: stall?.name,
...
export function updateProduct(id: string, updateData: Partial<Product>): Product | null {
  ...
  if (updateData.isVeg !== undefined) product.isVeg = Boolean(updateData.isVeg);
  if (updateData.isAvailable !== undefined) product.isAvailable = Boolean(updateData.isAvailable);
  if (updateData.imageUrl !== undefined) product.imageUrl = updateData.imageUrl.trim();
  if (updateData.badge !== undefined) product.badge = updateData.badge.trim() || undefined;
  if (updateData.preparationTimeMins !== undefined) product.preparationTimeMins = Number(updateData.preparationTimeMins);

  return product;
}

export function toggleProductAvailability(id: string, isAvailable: boolean): Product | null {
  const product = products.find(p => p.id === id);
  if (!product) return null;
...
// ---------------------------------------------------------------------------
// RIDER - VENDOR PICKUP OTP VERIFICATION SYSTEM
// ---------------------------------------------------------------------------
export const pickupVerifications = new Map<string, PickupVerificationRecord>();
export const pickupAuditLogs: PickupAuditLog[] = [];
export const deliveryAssignmentAudits: DeliveryAssignmentAudit[] = [];
export const deliveryVerifications = new Map<string, DeliveryVerificationRecord>();
export const onlineRiderIds = new Set<string>(['usr-deliv-01', 'usr-deliv-02']);
const PICKUP_OTP_TTL_MS = 10 * 60 * 1000;
const PICKUP_OTP_MAX_ATTEMPTS = 5;
const PICKUP_OTP_SECRET = process.env.PICKUP_OTP_SECRET || 'khau-katta-development-otp-secret';

function encryptPickupOtp(otpCode: string): string {
  const key = crypto.createHash('sha256').update(PICKUP_OTP_SECRET).digest();
  const iv = crypto.randomBytes(16);
  const cipher = crypto.createCipheriv('aes-256-cbc', key, iv);
  const encrypted = Buffer.concat([cipher.update(otpCode, 'utf8'), cipher.final()]);
  return `${iv.toString('hex')}:${encrypted.toString('hex')}`;
}

function decryptPickupOtp(ciphertext: string): string {
...
export function revealPickupOtp(record: PickupVerificationRecord): string {
  return decryptPickupOtp(record.otpCiphertext);
}

// Helper: Generate 6-digit numeric string
function generate6DigitOtpCode(): string {
  return Math.floor(100000 + Math.random() * 900000).toString();
}

/**
 * Retrieves existing active OTP for an order, or creates a new 6-digit OTP (10 min TTL)
 */
export function getOrCreatePickupOtp(orderId: string, stallId: string): PickupVerificationRecord {
  const order = orders.find(item => item.id === orderId);
  const assignedPartnerId = order?.deliveryPartnerId;
  const existing = pickupVerifications.get(orderId);
  const now = Date.now();

  // If exists, not verified, and not expired
  if (existing && !existing.isVerified && existing.expiresAt > now && existing.attempts < PICKUP_OTP_MAX_ATTEMPTS) {
    return existing;
  }

  // Create new OTP
  const otpCode = generate6DigitOtpCode();
  const record: PickupVerificationRecord = {
    id: `puv-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
    orderId,
    stallId,
    otpCiphertext: encryptPickupOtp(otpCode),
    deliveryPartnerId: assignedPartnerId,
    expiresAt: now + PICKUP_OTP_TTL_MS,
    attempts: 0,
    isVerified: false,
    status: 'ACTIVE',
    createdAt: new Date().toISOString(),
  ...
  pickupVerifications.set(orderId, record);

  // Log audit event
  pickupAuditLogs.push({
    id: `log-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
    orderId,
    stallId,
    action: 'OTP_GENERATED',
    details: '6-digit Pickup OTP generated for vendor (10-minute expiry)',
...
/**
 * Vendor-initiated regeneration of 6-digit Pickup OTP
 */
export function regeneratePickupOtp(orderId: string, stallId: string): PickupVerificationRecord {
  const previous = pickupVerifications.get(orderId);
  ...
  const record: PickupVerificationRecord = {
    ...
    stallId,
    otpCiphertext: encryptPickupOtp(otpCode),
    deliveryPartnerId: orders.find(item => item.id === orderId)?.deliveryPartnerId,
    expiresAt: now + PICKUP_OTP_TTL_MS,
    attempts: 0,
    isVerified: false,
    status: 'ACTIVE',
  ...
  pickupAuditLogs.push({
    ...
    details: 'Vendor requested a new 6-digit Pickup OTP',
    timestamp: new Date().toISOString()
  });
...
/**
 * Rider submits OTP to verify pickup from vendor
 */
export function verifyPickupOtp(
  orderId: string,
  riderId: string,
  inputOtp: string
): { success: boolean; message: string; remainingAttempts?: number; order?: Order } {
  const order = orders.find(o => o.id === orderId);
  if (!order) {
    return { success: false, message: 'Order not found in system.' };
  }

  if (!['ready_for_pickup', 'rider_arrived_at_vendor'].includes(order.status)) {
    if (order.status === 'picked_up' || order.status === 'out_for_delivery' || order.status === 'delivered') {
      return { success: false, message: 'Pickup has already been verified for this order.' };
    }
    return { success: false, message: `Cannot verify pickup for order in "${order.status.replace('_', ' ')}" status. Order must be marked "Ready for Pickup" by vendor.` };
  }

  // Get or create record
  let record = pickupVerifications.get(orderId);
  if (!record) {
    record = getOrCreatePickupOtp(orderId, order.stallId);
  }

  const now = Date.now();

  if (record.deliveryPartnerId && record.deliveryPartnerId !== riderId) {
    return { success: false, message: 'This order is assigned to another delivery partner.' };
  }

  if (record.blockedUntil && record.blockedUntil > now) {
    return { success: false, message: 'Pickup verification is temporarily blocked. Please contact the vendor or admin.' };
  }
  ...
  if (record.attempts >= 5) {
    ...
    pickupAuditLogs.push({
      ...
      details: 'Failed pickup verification: Maximum 5 attempts exceeded.',
      timestamp: new Date().toISOString()
    });
    return {
      success: false,
      message: 'Maximum verification attempts (5) reached. Please ask the vendor to regenerate a new OTP on their dashboard.',
      remainingAttempts: 0
    };
  }
  ...
  if (now > record.expiresAt) {
    ...
    pickupAuditLogs.push({
      ...
      riderId,
      action: 'OTP_EXPIRED',
      details: 'Pickup verification rejected because the OTP validity window elapsed.',
      timestamp: new Date().toISOString()
    });
    return {
      success: false,
      message: 'Pickup OTP has expired (10-minute time limit). Please ask the vendor to generate a new OTP.',
      remainingAttempts: 5 - record.attempts
    };
  }

  // Check matching OTP code without storing the code in plaintext.
  const trimmedInput = inputOtp.trim();
  const expectedOtp = decryptPickupOtp(record.otpCiphertext);
  if (expectedOtp !== trimmedInput) {
    record.attempts += 1;
    record.updatedAt = new Date().toISOString();
    ...
    pickupAuditLogs.push({
      ...
      details: `Incorrect OTP entered by rider. ${remaining} attempts remaining.`,
      timestamp: new Date().toISOString()
    });

    return {
      success: false,
      message: `Invalid OTP code. ${remaining > 0 ? `${remaining} attempts remaining.` : 'Maximum attempts reached.'}`,
  ...
  record.verifiedByRiderId = riderId;
  record.updatedAt = new Date().toISOString();

  // Update order status to 'picked_up'
  order.status = 'picked_up';
  order.pickupVerifiedAt = record.verifiedAt;
  order.pickupVerifiedBy = riderId;
  order.updatedAt = record.verifiedAt;
  order.statusHistory = [...(order.statusHistory || []), { status: 'picked_up', timestamp: order.updatedAt }];

  // Find rider name if available
  const riderUser = users.find(u => u.id === riderId);
  if (riderUser) {
    order.deliveryPartnerId = riderUser.id;
    order.deliveryPartnerName = riderUser.name;
    order.deliveryPartnerPhone = riderUser.phone;
  }
  ...
  pickupAuditLogs.push({
    ...
    details: `Pickup verified successfully by rider ${riderUser?.name || riderId} at stall. Status updated to "Picked Up".`,
    timestamp: new Date().toISOString()
  });

  return {
    success: true,
    message: '✅ Pickup OTP verified successfully! Order is now marked as Picked Up.',
    order
  };
}

/**
 * Returns pickup verification audit details for Admin or Order summary
 */
export function getPickupAuditDetailsForOrder(orderId: string): {
  isVerified: boolean;
  verifiedAt?: string;
  riderName?: string;
  stallName?: string;
  ...
  const order = orders.find(o => o.id === orderId);
  const record = pickupVerifications.get(orderId);
  const stall = stalls.find(s => s.id === order?.stallId);

  if (order?.status === 'picked_up' || order?.status === 'out_for_delivery' || order?.status === 'delivered') {
    return {
      isVerified: true,
      verifiedAt: order.pickupVerifiedAt || record?.verifiedAt,
      riderName: order.deliveryPartnerName || 'Ramesh Naik (Delivery Partner)',
      stallName: stall?.name,
      statusText: `✅ Pickup Verified ${order.pickupVerifiedAt ? 'at ' + new Date(order.pickupVerifiedAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : ''}`
    };
  }
  ...
  if (record) {
    return {
      ...
      stallName: stall?.name,
      statusText: `⏳ Pending Pickup OTP (${record.attempts}/5 attempts used)`
    };
  }
  ...
  return {
    ...
    attempts: 0,
    stallName: stall?.name,
    statusText: '⏳ Pending Pickup OTP Generation'
  };
}

// ---------------------------------------------------------------------------
// REAL-TIME GPS TRACKING, ROUTE OPTIMIZATION & ML PREDICTION ENGINE
// ---------------------------------------------------------------------------
...
export function predictDeliveryDelay(
  distanceKm: number,
  prepTimeMins: number = 10,
  trafficLevel: 'CLEAR' | 'MODERATE' | 'HEAVY' = 'MODERATE',
  weather: 'SUNNY' | 'RAIN' = 'SUNNY'
): MLDeliveryPrediction {
  let riskScore = Math.min(100, Math.round(distanceKm * 12 + prepTimeMins * 2));

  if (trafficLevel === 'HEAVY') riskScore += 30;
  if (trafficLevel === 'MODERATE') riskScore += 10;
  if (weather === 'RAIN') riskScore += 20;

  const isDelayed = riskScore >= 55;
  const confidence = isDelayed ? Math.min(95, 60 + riskScore / 3) : Math.min(96, 75 + (100 - riskScore) / 4);
  const baseTime = Math.round(distanceKm * 3.5 + prepTimeMins);
  const predictedDuration = isDelayed ? Math.round(baseTime * 1.4) : baseTime;

  return {
    prediction: isDelayed ? 'POSSIBLE_DELAY' : 'ON_TIME',
    confidencePercentage: Math.round(confidence),
    predictedDurationMins: predictedDuration,
    riskScore,
    contributingFactors: [
      {
        factor: 'Belagavi Street Distance',
        impact: distanceKm > 4 ? 'HIGH' : 'LOW',
        description: `Distance to customer destination: ${distanceKm} km.`
      },
      {
        factor: 'Stall Preparation Time',
        impact: prepTimeMins > 15 ? 'HIGH' : 'MEDIUM',
        description: `Fresh dish preparation time: ${prepTimeMins} mins.`
      },
      {
        factor: 'Traffic Corridor Density',
        impact: trafficLevel === 'HEAVY' ? 'HIGH' : 'LOW',
        description: `Club Road & Camp corridor traffic: ${trafficLevel.toLowerCase()}.`
      }
    ]
  };
}

// Route Optimization Engine (Standard Dijkstra vs AI-Optimized A*/VRP)
export function generateRouteOptimization(
  startLat: number,
  startLng: number,
  endLat: number,
  endLng: number
): DeliveryRoute {
  // This is an offline demo route.  A number of intermediate points prevents
  // the rider simulation from jumping in straight, diagonal segments.
  const buildDemoPath = (bend: number, label: string): DeliveryWaypoint[] => {
    ...
    return Array.from({ length: points + 1 }, (_, index) => {
      ...
      // Offset perpendicular to the journey, fading in and out at each end.
      const offset = Math.sin(Math.PI * progress) * bend;
      const lat = startLat + latDelta * progress + lngDelta * offset;
      const lng = startLng + lngDelta * progress - latDelta * offset;
      return {
        lat: Number(lat.toFixed(6)),
        lng: Number(lng.toFixed(6)),
        name: index === 0 ? 'Vendor Stall Pickup' : index === points ? 'Customer Destination' : label
      };
    });
  };

  const standardPath = buildDemoPath(0.3, 'Standard route waypoint');
  const optimizedPath = buildDemoPath(-0.16, 'Optimized route waypoint');

  const standardDist = calculateDistanceKm(startLat, startLng, endLat, endLng) + 0.8;
  const optimizedDist = calculateDistanceKm(startLat, startLng, endLat, endLng) + 0.2;

  const stdTime = Math.round(standardDist * 4.2);
  const optTime = Math.round(optimizedDist * 3.2);

  const distanceSaved = Number((standardDist - optimizedDist).toFixed(2));
  const timeSaved = Math.max(1, stdTime - optTime);
  const fuelSaved = Number((distanceSaved * 0.04).toFixed(3)); // Liters saved

  return {
    standardPath,
    optimizedPath,
    standardDistanceKm: standardDist,
    optimizedDistanceKm: optimizedDist,
    standardDurationMins: stdTime,
    optimizedDurationMins: optTime,
    distanceSavedKm: Math.max(0.4, distanceSaved),
    timeSavedMins: timeSaved,
    fuelSavedLiters: Math.max(0.02, fuelSaved),
    algorithmUsed: 'VRP_MULTI_CONSTRAINT'
  };
}
...
export const deliveries: Delivery[] = [
  {
    ...
    orderNumber: 'KK-2026-00180',
    stallId: 'stall-01',
    stallName: 'Belgaum Kunda & Sweets House',
    stallLat: 15.8522,
    stallLng: 74.5042,
    customerUserId: 'usr-cust-01',
    customerName: 'Pooja Kulkarni',
    customerPhone: '9845012345',
    deliveryAddress: 'Flat 402, Sai Residency, Camp, Belagavi 590001',
    destinationLat: 15.8647,
    destinationLng: 74.5124,
    status: 'unassigned',
    currentLat: 15.8580,
    currentLng: 74.5080,
    speedKmh: 34,
    ...
    route: generateRouteOptimization(15.8522, 74.5042, 15.8647, 74.5124),
    mlPrediction: predictDeliveryDelay(3.2, 10, 'MODERATE', 'SUNNY'),
  ...
  },
  {
    ...
    orderNumber: 'KK-2026-00156',
    stallId: 'stall-03',
    stallName: 'Tilakwadi Kadak Chai & Bun Maska',
    stallLat: 15.8450,
    stallLng: 74.5100,
    customerUserId: 'usr-cust-01',
    customerName: 'Pooja Kulkarni',
    customerPhone: '9845012345',
    deliveryAddress: 'Suite 12, Khanapur Road, Tilakwadi, Belagavi 590006',
    destinationLat: 15.8410,
    destinationLng: 74.5190,
    status: 'unassigned',
    currentLat: 15.8450,
    currentLng: 74.5100,
    speedKmh: 0,
    ...
    simulationMode: false,
    simulationProgress: 0,
    route: generateRouteOptimization(15.8450, 74.5100, 15.8410, 74.5190),
    mlPrediction: predictDeliveryDelay(2.1, 8, 'CLEAR', 'SUNNY'),
    lastUpdated: new Date().toISOString(),
    assignedAt: new Date(Date.now() - 10 * 60 * 1000).toISOString()
  }
];

export function createDeliveryForOrder(order: Order, address: UserAddress): Delivery {
  const existing = deliveries.find(delivery => delivery.orderId === order.id);
  if (existing) return existing;

  const stallCoordinates: Record<string, [number, number]> = {
    'stall-01': [15.8522, 74.5042],
    'stall-02': [15.8552, 74.5074],
    'stall-03': [15.8450, 74.5100]
  };
  const [stallLat, stallLng] = stallCoordinates[order.stallId] || [15.8522, 74.5042];
  const addressCoordinates: Record<string, [number, number]> = {
    'addr-01': [15.8647, 74.5124],
    'addr-02': [15.8410, 74.5190]
  };
  const [destinationLat, destinationLng] = addressCoordinates[address.id] || [stallLat, stallLng];
  const customer = users.find(user => user.id === order.userId);
  const route = generateRouteOptimization(stallLat, stallLng, destinationLat, destinationLng);
  ...
  const delivery: Delivery = {
    ...
    stallId: order.stallId,
    stallName: stalls.find(stall => stall.id === order.stallId)?.name || 'Khau Katta Stall',
    stallLat,
    stallLng,
    customerUserId: order.userId,
    customerName: customer?.name || 'Customer',
    customerPhone: customer?.phone || '',
    deliveryAddress: order.deliveryAddress || `${address.addressLine1}, ${address.area}, ${address.city} - ${address.pincode}`,
    destinationLat: order.deliveryLatitude ?? destinationLat,
    destinationLng: order.deliveryLongitude ?? destinationLng,
    destinationAccuracy: order.locationAccuracy,
    status: 'unassigned',
    currentLat: stallLat,
    currentLng: stallLng,
    speedKmh: 0,
    ...
    simulationProgress: 0,
    route,
    mlPrediction: predictDeliveryDelay(route.optimizedDistanceKm, 10, 'MODERATE', 'SUNNY'),
    lastUpdated: now,
    assignedAt: now
  };
...
export function generateDeliveryOtp(orderId: string): DeliveryVerificationRecord {
  ...
  deliveryVerifications.set(orderId, record);
  return record;
}

export function getOrCreateDeliveryOtp(orderId: string): DeliveryVerificationRecord {
  const existing = deliveryVerifications.get(orderId);
  if (existing && !existing.isVerified && existing.expiresAt > Date.now() && existing.attempts < 5) return existing;
  return generateDeliveryOtp(orderId);
}

// Helper: Step Simulation for Demo Mode
export function stepDeliverySimulation(deliveryId: string): Delivery | null {
  const delivery = deliveries.find(d => d.id === deliveryId || d.orderId === deliveryId);
  if (!delivery) return null;

  let nextProgress = delivery.simulationProgress + 0.08;
  if (nextProgress >= 1.0) {
    nextProgress = 1.0;
    ...
    if (order) {
      ...
      order.updatedAt = delivery.deliveredAt;
  ...
  delivery.simulationProgress = nextProgress;

  // Interpolate position along route.optimizedPath
  const path = delivery.route.optimizedPath;
  if (path.length >= 2) {
    const totalSegments = path.length - 1;
    const scaledIndex = nextProgress * totalSegments;
    const segIndex = Math.min(totalSegments - 1, Math.floor(scaledIndex));
    const segmentProgress = scaledIndex - segIndex;
    ...
    delivery.currentLat = Number(currentLat.toFixed(6));
    delivery.currentLng = Number(currentLng.toFixed(6));
    delivery.headingDeg = calculateHeading(p1.lat, p1.lng, p2.lat, p2.lng);
    delivery.speedKmh = nextProgress < 1.0 ? Math.floor(26 + Math.random() * 12) : 0;
  }
  ...
  locationLogs.push({
    ...
    partnerId: delivery.partnerId || 'unassigned',
    latitude: delivery.currentLat,
    longitude: delivery.currentLng,
    accuracy: undefined,
    speed: delivery.speedKmh,
    heading: delivery.headingDeg,
...
// Helper: Rider GPS Update
export function updateDeliveryGps(
  deliveryId: string,
  lat: number,
  lng: number,
  speedKmh?: number,
  ...
  delivery.currentLocationAccuracy = accuracy;
  if (speedKmh !== undefined) delivery.speedKmh = speedKmh;
  if (headingDeg !== undefined) delivery.headingDeg = headingDeg;
  delivery.lastUpdated = new Date().toISOString();

...
export interface PickupAuditLog {
  ...
  riderId?: string;
  riderName?: string;
  action: 'OTP_GENERATED' | 'OTP_REGENERATED' | 'VERIFICATION_SUCCESS' | 'VERIFICATION_FAILED' | 'ATTEMPTS_EXCEEDED';
  details: string;
  timestamp: string;
}
...
export interface Delivery {
  ...
  status: 'assigned' | 'accepted' | 'ready_for_pickup' | 'picked_up' | 'out_for_delivery' | 'delivered' | 'cancelled';
  currentLat: number;
  currentLng: number;
  speedKmh: number;
  headingDeg: number;
